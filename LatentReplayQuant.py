...
from model.phinetv1_quant import PhiNetV1
...
def main():
    ...
    calibration_data = train_stream[0].dataset.subset(list(range(1000)))
    for exp in train_stream[1:]:
        calibration_data = calibration_data.concat(exp.dataset.subset(list(range(1000))))

    # Create the dataloaders
    train_loader = DataLoader(calibration_data, batch_size=64, shuffle=True)
    ...
    remove_depthwise(model)
    model = PhiNetV1(model=model, latent_layer_num=args.latent_layer, quantized=True)

    model.lat_features = quantize_model(model.lat_features, phinet_fuse_modules, calibrate_model, train_loader, quantized_output=True)
    model = model.to(device)
    model.lat_features = model.lat_features.to("cpu")
